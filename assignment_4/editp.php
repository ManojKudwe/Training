 ...
 <!--header-starts-->
  ...
  <div class="section content_section">
	<div class="container">
		...
		<form name="myForm" action="" enctype="multipart/form-data" onsubmit="return validatepro()" method="post">
...
$qry = mysqli_query($con,"select * from cate where pname='$id'"); 
...
if(isset($_POST['update'])) 
{
  $cate = $_POST['cate'];
    ...
    move_uploaded_file($_FILES["image"]["tmp_name"],$dst); 
	 ...
	 $edit = mysqli_query($con,"update cate set category='$cate', pname='$pname' , price='$price', pimage=' $dst_db ' where pname='$id'");
    if($edit)
    {
...
var ct = document.getElementById("pname").value;
var cpr = document.getElementById("price").value;
var letters = /^[A-Za-z]+$/;
var lt = /^[0-9]+$/;
if(ct!=null && cpr!=null){
    if(ct.match(letters)){  
        ...
        alert("Please Enter alphabets only in Product name!");
    return false;
	   }
    ...
    if(cpr.match(lt)){  
        ...
        alert("Please Enter digits only in price!");
    return false;
	   }
   }
}
				...
				<ul>
					<li class="fileds">
						<div class="name_fileds">
							...
							<input name="pname" id="pname"  type="text" value="<?php echo $data['pname'] ?>" type="text" /> 
						</div>
					</li>
					<li class="fileds">
						<div class="name_fileds">
							<label>Product Price</label>
							<input name="price" id="price" value="<?php echo $data['price'] ?>" type="text" /> 
						</div>
					</li>
					<li class="fileds">
						<div class="upload_fileds">
							<label>Upload Image</label>
							<input name="image" id="image" type="file" accept="image/x-png,image/jpg"  placeholder="Choose File" >
						</div>						
					</li>
<?php
...
$records = mysqli_query($con,"select * from categ");
{
?>
					<li class="fileds">
						<div class="name_fileds">
							<label>Select Category</label>
							<select name="cate" id="cate" class="select category">
							<?php while($data = mysqli_fetch_array($records)):;?>
			<option value="<?php echo $data[1]; ?>"<?php if ($data[1]== $dataa['category'])echo'selected="selected"';?>><?php echo $data[1]; ?></option>
							<?php endwhile;?>	
							</select> 
						</div>
					</li>
...
?>
				...
				<div class="next_btn_block">
					<div class="next_btn">
						<button type="submit" name="update" value="update" style="background-color: #9fbc35; border-color:transparent; padding: 20px; color: #fff; width: 230px; display: block;text-align: center; font-size: 18px; line-height: 25px; font-family: 'robotoslab-bold'; border-radius: 5px; margin: 0 auto; text-decoration: none; ">Submit<span><img src="images/small_triangle.png" alt="small_triangle"> </span></button>
					</div>
				</div>
			</div>
			</form>
