...
if(isset($_POST['delete'])){
	...
	$del_id = $checkbox[$i]; 
	mysqli_query($con,"DELETE FROM categ WHERE category='".$del_id."'");
...
$rpp = 3;  
        ...
        $sp = ($pg-1) * $rpp;         
        $records = mysqli_query ($con, "SELECT * FROM categ LIMIT $sp, $rpp");
while($data = mysqli_fetch_array($records))
{
?>
  <tr>
     <td width="10%"><input type="checkbox" id="checkItem" value="<?php echo $data['category']; ?>" name="chkbox[]" /></td>  
...
?>
      ...
      <?php  
        $rcount = mysqli_query($con, "SELECT COUNT(*) FROM categ");     
        ...
        if($pg>=2){   
            echo "<a href='index.php?pg=".($pg-1)."'>  Prev </a>";   
        }       
                   
        for ($i=1; $i<=$total_pgs; $i++) {   
          if ($i == $pg) {   
              $pagLink .= "<a class = 'active' href='index.php?pg="  
                                                .$i."'>".$i." </a>";   
          }               
          else  {   
              $pagLink .= "<a href='index.php?pg=".$i."'>   
                                                ".$i." </a>";     
          }   
        };     
        echo $pagLink;   
        ...
        if($pg<$total_pgs){   
            echo "<a href='index.php?pg=".($pg+1)."'>  Next </a>";   
        }   
